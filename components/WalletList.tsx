...
const WalletList = ({address}: {address: string}) => {
    ...
    <main className='flex flex-col justify-center items-center gap-5'>
      ...
      {wallets.length === 0 ? (
        <div className='flex justify-center items-center border-2 border-dashed p-6 rounded-lg'>
          <p className='text-lg'>You currently have no smart contract wallets</p>
        </div>
      ): (
        <div className='grid grid-cols-2 gap-4'>
          {wallets.map((wallet) => (
            <div key={wallet.id} className='flex flex-col border border-gray-800 rounded-lg'>
              <Link className='text-gray-300 font-medium' href={`/${wallet.address}`}>
                <div className='bg-gray-800 flex justify-between gap-4 items-center rounded-t-lg p-2 font-mono'>
                  {wallet.address}
                  <p className='text-gray-300'>Pending Txns: {wallet._count.transactions}</p>
